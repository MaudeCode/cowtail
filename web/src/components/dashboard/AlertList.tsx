  ...
  return (
    <div>
      ...
      >
        ...
        <span
          className={`font-mono text-[0.7rem] uppercase ${severityStyle[group.highestSeverity] || ""}`}
        >
          {group.highestSeverity}
        </span>
        <span
          className={`inline-block font-mono text-[0.65rem] uppercase tracking-[0.08em] whitespace-nowrap px-2 py-[3px] font-medium ${outcomeBg[outcomeKey] || ""}`}
        >
      ...
      >
        <div className="flex items-center justify-between mb-1 gap-2">
          ...
          <span
            className={`inline-block font-mono text-[0.6rem] uppercase tracking-[0.08em] whitespace-nowrap px-2 py-[3px] font-medium shrink-0 ${outcomeBg[outcomeKey] || ""}`}
          >
            {alert.outcome.replace("-", " ")}
          </span>
        </div>
...
export default function AlertList({
  ...
  return (
    <div className="px-10 max-lg:px-4">
      {/* Desktop header */}
      <div
        className={`${cols} py-3.5 border-b-2 border-gray-200 font-mono text-[0.65rem] uppercase tracking-[0.12em] text-gray-400 max-lg:hidden`}
      >
        <button
          type="button"
          className="cursor-pointer hover:text-txt select-none"
